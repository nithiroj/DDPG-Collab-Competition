...
parser.add_argument('--episodes', default=3500, help='Number of episodes', type=int)
parser.add_argument('--output', default='output', help='Location to output')

def plot_rewards(scores, queue_scores, output):
    plt.plot(np.arange(1, len(scores)+1), scores)
    plt.plot(np.arange(1, len(queue_scores)+1), queue_scores)
    plt.ylabel('Score')
    plt.xlabel('Episode #')
    plt.savefig(f'{output}/rewards.png')

def train(env_pth, n_episodes=3500, output='output'):
    ...
    def ddpg(n_episodes, average_window=100, output='output'):
        ...
        for i_episode in range(1, n_episodes+1):
            ...
            max_score_episode = np.max(scores)
            scores_deque.append(max_score_episode)
            scores_all.append(max_score_episode)
            average_score_queue = np.mean(scores_deque)
            if i_episode >= average_window:
                queue_scores.append(average_score_queue)
            else:
                queue_scores.append(0)

            logger.info(f'\rEpisode {i_episode}\tScore (max over agents): {max_score_episode}\tAverage Score: {average_score_queue}')
            ...
            if i_episode >= average_window and average_score_queue >= 0.5:
                break
        
        return scores_all, queue_scores

    scores, queue_scores = ddpg(n_episodes=n_episodes, output=output)
    plot_rewards(scores, queue_scores, output)
